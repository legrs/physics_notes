...
class LMStudio:
    def __init__(self, server: str, model: str, timeout: float = 300.0,
        ...
        # 実測（2026-07-08）: Qwen3.5-9B は既定で1トリビアルな質問にも数百
        # reasoning token を使い、実プロンプトでは max_tokens 4096 を使い切って
        # 本文が空になった（432秒かけて空応答）。reasoning_effort="none" を
        # 渡すと reasoning_tokens=0 になり、9B で 12秒、1.7B で 0.8秒に激減。
        # 言い換え生成・語の提案はどちらも多段推論を要らないタスクなので既定でOFF。
        # 対応していないモデルは未知フィールドとして無視される想定（None で明示的に外せる）。
...
# コーパス中でこの割合を超えて既に出現する語は「一般的すぎる語」とみなして
# 機械的に却下する。プロンプトで明示的に禁止しても（"理由"はまさに悪い例として
# 挙げている）小型・非thinkingモデルは守らないことがあるため、記述的な指示だけに
# 頼らずコーパス頻度で客観的にガードする。220件規模で ratio=0.03 は ~6-7件が閾値。
GENERIC_TERM_MAX_RATIO = 0.03
GENERIC_TERM_MIN_ABS = 3  # 小さいコーパスでも最低これだけは許容


def corpus_term_count(term: str, all_records: list[dict]) -> int:
    """search_text に既にこの語（小文字化・部分一致）を含むレコード数。"""
    t = term.lower()
    return sum(1 for r in all_records if t in (r.get("search_text") or "").lower())


def is_too_generic(term: str, all_records: list[dict]) -> bool:
    if not all_records:
        return False
    threshold = max(GENERIC_TERM_MIN_ABS, int(len(all_records) * GENERIC_TERM_MAX_RATIO))
    return corpus_term_count(term, all_records) > threshold
    ...
    for field in ("keywords", "synonyms"):
        ...
        for v in vals:
            if not isinstance(v, str):
                continue
            term = v.strip()
            if not valid_term(term, existing):
                continue
            if is_too_generic(term, all_records):
                continue  # コーパス中で頻出しすぎる語（例: 「理由」）は却下
...
def parse_args():
    ...
    ap.add_argument("--reasoning-effort", default="none",
                    help='LLM の reasoning_effort（既定 "none" — 言い換え/語の提案は多段推論'
                         '不要なうえ、thinkingモデルでは思考だけでmax_tokensを使い切り本文が'
                         '空になることがある。空文字列を渡すとフィールド自体を送らない）')
